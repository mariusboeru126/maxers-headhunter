...
export async function upsertUserFromGoogleProfile(profile) {
  const rows = await query("SELECT id, full_name, email, is_active FROM users WHERE email = ?", [profile.email]);
  let user = rows[0];
  ...
  if (!user) {
    ...
    user = { id: result.insertId, full_name: fullName, email: profile.email, is_active: 1 };
  }
...
export function buildAuthSessionCookie(user) {
  ...
  return `${AUTH_COOKIE_NAME}=${token}; Path=/; HttpOnly; SameSite=Lax${secureSuffix}; Max-Age=${maxAge}`;
}
